...
class ImageAnalyzer: ImageAnalyzerService {
    ...
    func analyzeImage(image: UIImage, navigationController: UINavigationController, failure: @escaping (_ error: String) -> Void) {
        ...
        guard let model = try? VNCoreMLModel(for: Analisador(configuration: MLModelConfiguration()).model) else {
            failure("can't load Analisador ML model")
            return
        }
        ...
        // Run the Core ML Analisador classifier on global dispatch queue
